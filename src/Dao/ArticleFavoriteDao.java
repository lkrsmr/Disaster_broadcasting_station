...
public class ArticleFavoriteDao {
    public Set<Integer> getArticleFavorite (String username){
        ...
        try {
            ...
            sql.append("select article_id from articles_favorites ")
            ...
            while(rs.next()){
                favoriteSet.add(rs.getInt("article_id"));
            }
        }catch (Exception e) {
            e.printStackTrace();
        }finally {
    ...
    public boolean deleteArticleFavorite (int id) {
        Connection conn =null;
        Statement stmt = null;
        ResultSet rs = null;
        //删除操作后受影响的行数
        ...
        try {
            ...
            // 而stringBuffer是初始化创建一次，之后每次在后面追加内容，不需要重新开辟内存空间，提高代码执行效率。
            StringBuffer sql = new StringBuffer(200);
            sql.append("delete from articles_favorites ")
    ...
    public boolean addArticleFavorite (String username ,int id) {
        Connection conn =null;
        Statement stmt = null;
        ResultSet rs = null;
        //插入操作后受影响的行数
        ...
        try {
            ...
            // 而stringBuffer是初始化创建一次，之后每次在后面追加内容，不需要重新开辟内存空间，提高代码执行效率。
            StringBuffer sql = new StringBuffer(200);
            sql.append("insert into articles_favorites value (")
                    .append("'").append(username).append("'")
                    .append(",")
                    .append(id).append(")");
            System.out.println(sql);
