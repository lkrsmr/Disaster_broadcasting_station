...
<nav class="navbar-default navbar-fixed-top navbar-shrink " style="height:110px;">
    ...
    <div class="neck">
        <div class="neck-items">
            <ul>
                <!-- 现在是主页，因此把active的css样式给首页标签 -->
                <li class="active"><a href="infos.jsp">首页</a></li>
...
<div class="main">
    ...
    <div class="right">
        <div class="right-title">
            <span class="right-title-text">科普文章排行</span>
        </div>
        <% List<ArticleBean> rankList = new ArrayList<ArticleBean>();
        ArticleDao rankDao = new ArticleDao();
        //选择显示文章的条目数量
        rankList = rankDao.getArticleRank(5);%>

        <div class="article-wrap">
            <ul>
<c:forEach items="<%= rankList %>" var = "article">
                <li class="item">
                    <a href="AddArticleHotServlet?id=${article.id}&url=${article.url}">${article.title}</a>
                    <span class="hot">${article.hot}阅读</span>
                </li>
</c:forEach>
            </ul>
        </div>
    ...
    <div class="container main-container">
        ...
        //首页只显示最新的5条数据
            ArrayList<InfoBean> infoList = (ArrayList<InfoBean>) infoDao.getLeastInfos(5); %>
            ...
            <div class="newContentBox">
                ...
                <% } else {
                    //如果保存收藏列表的set中存在此次输出的id，说明这条信息在用户收藏中
                        ...
                        <a  href="DeleteInfoFavoriteServlet?id=${info.id}&src=1">
                             <img class="favoriteIcon" src="static/images/cancel-favorite.png">
                        </a>
                        <%} else { //不存在，该条信息未被收藏 %>
                        <a  href="AddInfoFavoriteServlet?id=${info.id}&src=1">
                            <img class="favoriteIcon" src="static/images/favorite.png">
                        </a>
                <%      }
                    }
        ...
        <div class="more"><a href="infos.jsp">>>>>>>>查看更多>>>>>>></a></div>
    </div>

    <div class="container main-container">
        <div class="bt">科普文章</div>
        <% ArticleFavoriteDao articleFavoriteDao = new ArticleFavoriteDao();
            HashSet<Integer> articleFavoriteSet = null;
            //还未登录，则不获取收藏数据
            if(username != null){
                is_login = true;
                articleFavoriteSet = (HashSet<Integer>) articleFavoriteDao.getArticleFavorite(username);
                //存入request，通过jsp标签<c:if>使用
                request.setAttribute("articleFavoriteSet",articleFavoriteSet);
            }
        %>
        <% ArticleDao articleDao = new ArticleDao();
            //首页只显示最新的5条数据
            ArrayList<ArticleBean> articleList = (ArrayList<ArticleBean>) articleDao.getLeastArticles(5); %>
<c:forEach items="<%= articleList %>" var = "article">
    <c:set var="article_id" value="${article.id}" scope = "request"></c:set>
        <div class="newContentBox">
            <a hidefocus="true" href="AddArticleHotServlet?id=${article.id}&url=${article.url}">
                ...
                <% } else {
                    //如果保存收藏列表的set中存在此次输出的id，说明这条信息在用户收藏中
                    int id = (int) request.getAttribute("article_id");
                    if (articleFavoriteSet.contains(id)) { %>
                <a  href="DeleteArticleFavoriteServlet?id=${article.id}&src=1">
                    <img class="favoriteIcon" src="static/images/cancel-favorite.png">
                </a>
                <%} else { //不存在，该条信息未被收藏 %>
                <a  href="AddArticleFavoriteServlet?id=${article.id}&src=1">
                    <img class="favoriteIcon" src="static/images/favorite.png">
                </a>
                <%      }
                }
        ...
        <div class="more"><a href="articles.jsp">>>>>>>>查看更多>>>>>>></a></div>
    </div>
